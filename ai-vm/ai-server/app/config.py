...
AI_VM_ROOT = Path(__file__).resolve().parents[2]
STORAGE_ROOT = Path(os.environ.get("AI_STORAGE_ROOT", SERVICE_ROOT / "storage"))
EVENT_MEDIA_DIR = Path(os.environ.get("EVENT_MEDIA_DIR", STORAGE_ROOT / "event_media"))
ROI_SETTINGS_PATH = Path(os.environ.get("ROI_SETTINGS_PATH", STORAGE_ROOT / "roi_settings.json"))


def load_env_file(path: Path | str | None = None) -> None:
    env_paths = [Path(path)] if path else [SERVICE_ROOT / ".env", AI_VM_ROOT / ".env"]
...
MJPEG_STREAM_FPS = float(os.environ.get("MJPEG_STREAM_FPS", "5.0"))
