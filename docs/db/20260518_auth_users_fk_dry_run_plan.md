# Auth/signup -> users FK dry-run plan
...
Validate whether auth/signup tables can remove direct foreign key dependencies on `users.id`.
...
The auth/signup candidate check confirmed that `email_verifications.user_id` and `signup_requests.user_id` exist in both `staccato` and `staccato_test`.
...
|---|---|---|---|
| `email_verifications` | `user_id` | `email_verifications_ibfk_1` | `users.id` |
| `signup_requests` | `user_id` | `signup_requests_ibfk_1` | `users.id` |
...
2. Run `13_check_auth_users_fk.sql`.
3. Confirm both target FKs exist.
4. Confirm all `orphan_count` values are `0`.
5. Run `14_drop_auth_users_fk.sql` on `staccato_test` only.
6. Confirm both target FKs are removed.
7. Run tests.
8. Run `15_rollback_auth_users_fk.sql`.
...
- `email_verifications_ibfk_1` exists before drop.
- `signup_requests_ibfk_1` exists before drop.
- All orphan checks return `0`.
- FK drop SQL succeeds on `staccato_test`.
- `email_verifications.user_id` column remains.
- `signup_requests.user_id` column remains.
