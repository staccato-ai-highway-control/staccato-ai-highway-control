"""board support models 도메인의 SQLAlchemy 영속성 모델을 정의한다.
...
# 설명: `BoardReaction` 클래스를 정의하고 db.Model의 동작 또는 계약을 확장한다.
class BoardReaction(db.Model):
    # 설명: `__tablename__`의 기준값 또는 기본값을 'board_reactions'로 설정한다.
    ...
    # 설명: `reaction_type`에 `db.Column` 호출 결과를 저장해 다음 처리에서 사용한다.
