...
@require_auth
def list_all_report_model_comparison_batches():
    try:
        page = request.args.get("page", 1, type=int)
        size = request.args.get("size", 20, type=int)
        status = request.args.get("status")

        result, status_code = ReportModelComparisonService.list_all_batches(
        ...
        logger.exception("전체 모델 비교 분석 목록 조회 중 오류 발생")
        return jsonify({
            "success": False,
