      ...
      <AppLayout title="신고 상세">
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <main className="grid min-w-0 content-start gap-6">
            ...
            {annotatedMediaUrl ? <MediaPreviewCard title={`AI 객체 탐지 결과${annotatedMediaType === "video" ? " 영상" : " 이미지"}`} actions={<><a href={annotatedMediaUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 no-underline hover:bg-slate-50">새 탭 열기</a><a href={annotatedMediaUrl} download className="inline-flex min-h-9 items-center rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 no-underline hover:bg-slate-50">다운로드</a></>}>{annotatedVideoUrl ? <RetryableVideo src={annotatedVideoUrl} controls playsInline errorMessage="AI 탐지 결과 영상을 불러오지 못했습니다." /> : <img src={annotatedImageUrl ?? ""} alt="AI 객체 탐지 결과" />}</MediaPreviewCard> : null}
