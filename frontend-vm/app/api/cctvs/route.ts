...
function normalizeCctv(item: any, index: number) {
  ...
  // 코드 설명: sourceUrl 값은 원본 보존 용도로만 사용하고 브라우저 stream URL에는 노출하지 않습니다.
  ...
  return {
    ...
    streamUrl: `/api/cctvs/${encodeURIComponent(displayCode)}/stream`,
    stream_url: `/api/cctvs/${encodeURIComponent(displayCode)}/stream`,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstreamUrl = new URL("/api/cctvs", FLASK_API_BASE_URL.replace(/\/$/, ""));
    request.nextUrl.searchParams.forEach((value, key) => {
      upstreamUrl.searchParams.set(key, value);
    });
    upstreamUrl.searchParams.set("source", "ai-vm");
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: `CCTV Gateway API failed: ${response.status}`,
          count: 0,
          rawCount: 0,
          selectedCount: 0,
          limit: DEFAULT_LIMIT,
    ...
    return NextResponse.json(
      {
        success: false,
        message: "CCTV metadata fetch failed",
        count: 0,
        rawCount: 0,
        selectedCount: 0,
        limit: DEFAULT_LIMIT,
