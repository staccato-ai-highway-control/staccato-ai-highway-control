...
export default function AdminBoardPage() {
  ...
  async function loadPosts() {
    ...
    try {
      const nextPosts = await getBoardPosts({ page: 1, size: 100 });
      setPosts(nextPosts);
    } catch (error) {
    ...
    return posts
      ...
      .filter((post) => {
        ...
        const matchesQuery =
          ...
          String(post.author_id).includes(normalizedQuery);
      ...
      <AppLayout title="관리자 게시판">
        ...
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            ...
            <span className="text-sm font-semibold text-slate-500">{loading ? "불러오는 중" : `${filteredPosts.length}건`}</span>
          </div>
