...
import { AlertTriangle, ArrowRight, Bell, CheckCircle2, Clock3, Cctv, FileText, Radio, RefreshCw, ShieldAlert, UserCheck, Users } from "lucide-react";
...
function StatGrid({ cards }: { cards: StatCard[] }) {
  ...
  return <section className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-4">{cards.map((card) => { const Icon = card.icon; return <article key={card.label} className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-[0_14px_34px_-28px_rgba(15,23,42,0.5)] transition duration-200 hover:-translate-y-0.5 hover:border-slate-300"><div className="flex items-start justify-between gap-3"><span className={`grid h-10 w-10 place-items-center rounded-xl ${card.bg}`}><Icon className={`h-5 w-5 ${card.tone}`} aria-hidden="true" /></span><span className="rounded-full bg-slate-50 px-2.5 py-1 text-[11px] font-bold text-slate-400">{card.helper}</span></div><div className="mt-5 flex items-end justify-between gap-3"><div><strong className="block text-3xl font-black tracking-tight text-slate-950">{card.value}</strong><p className="mt-1 text-sm font-bold text-slate-500">{card.label}</p></div><span className="h-1.5 w-10 rounded-full bg-slate-100 transition group-hover:bg-sky-200" /></div></article>; })}</section>;
}
...
function IncidentRows({ incidents }: { incidents: Incident[] }) {
  ...
  // 코드 설명: 현재 상태와 권한 조건을 반영한 JSX 화면 구조를 호출한 React 렌더러에 반환합니다.
  return <div className="grid gap-2.5">{incidents.map((incident) => <Link key={incident.id} href={`/incidents/${incident.id}`} className="group grid gap-3 rounded-xl border border-slate-100 bg-slate-50/40 px-4 py-3.5 text-slate-900 no-underline transition hover:border-sky-200 hover:bg-sky-50/50 md:grid-cols-[1fr_auto_auto_auto] md:items-center"><div className="min-w-0"><b className="block truncate text-sm text-slate-950">{incident.title}</b><p className="mt-1 truncate text-xs font-semibold text-slate-500">{incidentTypeLabels[incident.eventType]} · {incident.location}</p></div><RiskLevelBadge level={incident.riskLevel} /><IncidentStatusBadge status={incident.status} /><ArrowRight className="hidden h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-sky-600 md:block" aria-hidden="true" /></Link>)}</div>;
}
...
function ReportRows({ reports }: { reports: Report[] }) {
  ...
  // 코드 설명: 현재 상태와 권한 조건을 반영한 JSX 화면 구조를 호출한 React 렌더러에 반환합니다.
  return <div className="grid gap-3 md:grid-cols-2">{reports.map((report) => <Link key={report.id} href={`/reports/${report.id}`} className="group flex items-center justify-between gap-4 rounded-xl border border-slate-100 bg-slate-50/40 p-4 text-slate-900 no-underline transition hover:border-sky-200 hover:bg-sky-50/50"><div className="min-w-0"><b className="block truncate text-sm text-slate-950">{getReportTitle(report)}</b><p className="mt-1 truncate text-xs font-semibold text-slate-500">{getReportCode(report)} · {report.status ?? "SUBMITTED"} · {getReportLocation(report)}</p></div><ArrowRight className="h-4 w-4 shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-sky-600" aria-hidden="true" /></Link>)}</div>;
}
...
function CctvStatusGrid({ cctvs }: { cctvs: CctvItem[] }) {
  ...
  // 코드 설명: 현재 상태와 권한 조건을 반영한 JSX 화면 구조를 호출한 React 렌더러에 반환합니다.
  return <div className="grid grid-cols-3 gap-2.5">{(["ONLINE", "OFFLINE", "MAINTENANCE"] as const).map((status) => { const count = cctvs.filter((cctv) => cctv.status === status).length; return <div key={status} className="rounded-xl border border-slate-100 bg-slate-50/60 px-2 py-4 text-center"><Cctv className="mx-auto h-5 w-5 text-slate-400" aria-hidden="true" /><strong className="mt-2 block text-2xl font-black text-slate-950">{count}</strong><p className="mt-1"><Badge className="min-h-5 px-2 text-[10px]" tone={status === "ONLINE" ? "green" : status === "OFFLINE" ? "red" : "amber"}>{status}</Badge></p></div>; })}</div>;
}
...
export default function DashboardPage() {
  ...
  // 코드 설명: 현재 상태와 권한 조건을 반영한 JSX 화면 구조를 호출한 React 렌더러에 반환합니다.
  return <RequireAuth><AppLayout title="대시보드" wide>
    <section className="mb-6 overflow-hidden rounded-2xl border border-slate-200/80 bg-white px-5 py-5 shadow-[0_18px_45px_-34px_rgba(15,23,42,0.55)] sm:px-6 lg:flex lg:items-center lg:justify-between lg:gap-8"><div className="flex items-start gap-4"><span className="hidden h-12 w-12 shrink-0 place-items-center rounded-2xl bg-slate-950 text-sky-300 sm:grid"><Radio className="h-6 w-6" aria-hidden="true" /></span><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-2xl font-black tracking-tight text-slate-950">통합 관제 대시보드</h2><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-black text-emerald-700"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />LIVE</span></div><p className="mt-1.5 text-sm font-medium text-slate-500">이벤트, 신고, CCTV 운영 현황을 한눈에 확인하세요.</p></div></div><div className="mt-5 flex flex-wrap gap-2 lg:mt-0 lg:justify-end"><button type="button" onClick={handleRealtimePreviewClick} disabled={isPreviewLoading} className="inline-flex h-10 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-50"><Radio className="h-4 w-4" aria-hidden="true" />{isPreviewLoading ? "불러오는 중" : "실시간 미리보기"}</button><Link href="/incidents" className="inline-flex h-10 items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 no-underline transition hover:bg-slate-50">이벤트 관리</Link><Link href="/reports" className="inline-flex h-10 items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 no-underline transition hover:bg-slate-50">신고 관리</Link><button type="button" onClick={loadDashboard} className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:bg-slate-50"><RefreshCw className="h-4 w-4" aria-hidden="true" />새로고침</button></div></section>
    ...
    <section className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><DashboardPanel compact title="최근 이벤트 요약" titleAction={<Link href="/incidents" className="inline-flex items-center gap-1 text-xs font-bold text-sky-700 no-underline">전체 보기<ArrowRight className="h-3.5 w-3.5" aria-hidden="true" /></Link>}><SectionState state={incidentState} loadingText="최근 이벤트를 불러오는 중입니다."><IncidentRows incidents={incidents.filter(isUnresolvedIncident).slice(0, 5)} /></SectionState></DashboardPanel><div className="grid gap-5"><DashboardPanel compact title="CCTV 상태 요약" titleAction={<Link href="/cctvs" className="inline-flex items-center gap-1 text-xs font-bold text-sky-700 no-underline">관제 보기<ArrowRight className="h-3.5 w-3.5" aria-hidden="true" /></Link>}><SectionState state={cctvState} loadingText="CCTV 상태를 불러오는 중입니다."><CctvStatusGrid cctvs={cctvs} /></SectionState></DashboardPanel><DashboardPanel compact title="처리 대기 이벤트"><SectionState state={incidentState} loadingText="처리 대기 이벤트를 불러오는 중입니다."><Link href="/incidents" className="group flex items-center gap-4 rounded-xl border border-amber-200/80 bg-gradient-to-br from-amber-50 to-orange-50 p-4 text-amber-950 no-underline transition hover:border-amber-300"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-white text-amber-700 shadow-sm"><Clock3 className="h-5 w-5" aria-hidden="true" /></span><span className="min-w-0 flex-1"><strong className="block text-2xl font-black">{incidents.filter((incident) => ["DETECTED", "REVIEWING", "ASSIGNED"].includes(incident.status)).length}</strong><span className="block text-xs font-bold text-amber-700">확인 또는 조치가 필요합니다</span></span><ArrowRight className="h-5 w-5 text-amber-400 transition group-hover:translate-x-0.5" aria-hidden="true" /></Link></SectionState></DashboardPanel></div></section>
    <section className="mt-5"><DashboardPanel compact title="최근 신고 목록" titleAction={<Link href="/reports" className="inline-flex items-center gap-1 text-xs font-bold text-sky-700 no-underline">전체 보기<ArrowRight className="h-3.5 w-3.5" aria-hidden="true" /></Link>}><SectionState state={reportState} loadingText="최근 신고를 불러오는 중입니다."><ReportRows reports={reports.slice(0, 5)} /></SectionState></DashboardPanel></section>
